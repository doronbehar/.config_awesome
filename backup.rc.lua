...
--mywidgets.musicplayer = obvious.basic_mpd()
-- Keyboard map indicator and switcher
mywidgets.keyboardlayout = awful.widget.keyboardlayout()
-- }}}
-- {{{ Menu
...
awful.screen.connect_for_each_screen(function(s)
	...
	-- Create the wibox
	s.mywibox = awful.wibox({ position = "top", height ="20", screen = s })
	-- Add widgets to the wibox of the second/first screen
	if s == math.min(screen.count(), 2) then
		s.mywibox:setup {
			layout = wibox.layout.align.horizontal,
			-- Left widgets for second/first screen
			{
			...
			-- Right widgets for second/first screen
			{
				layout = wibox.layout.fixed.horizontal,
				mywidgets.volume,
				mywidgets.divider,
	...
	if s == 1 then
		s.mywibox:setup {
			...
			-- Right widgets for first screen
			{
				layout = wibox.layout.fixed.horizontal,
				--mywidgets.musicplayer
				mywidgets.clock,
				mykeyboardlayout,
				wibox.widget.systray(),
				s.mylayoutbox,
...
clientkeys = awful.util.table.join(
	...
	-- {{{ Clients Maximization and Minimaization
